...
function subAttendanceWindow(m, sub) {
  ...
  if (m && Array.isArray(m.subscriptions) && sub.start) {
    ...
    // FILL UP TO PAID CLASSES (v6.433). When there is NO LATER package for this sport, this is the
    // current/last one — it owns attendance from `from` onward, but only up to the PAID class count.
    // The window's END is moved to the date the member reached their class limit, so a class attended
    // shortly AFTER the validity date fills the last paid slot (e.g. a 12th class on 23 Jul, 2 days
    // after a 21 Jul expiry → 12/12), and NEVER beyond it — a member who kept attending many extra
    // classes after the package was used up stays capped at their paid count (card + coach commission
    // both cap). If they haven't used all their classes yet, the window is left open (still < limit).
    if (!sameAct.length && m.dailyAttendance) {
      ...
      if (limit > 0) {
        ...
        marks.sort();
        // ≥ limit marks → end the window on the limit-th class (fills up to paid, caps the rest).
        // < limit marks → leave the window open (they still have unused paid classes).
        to = (marks.length >= limit) ? marks[limit - 1] : null;
      }
    }
  }
  return { from, to };
...
function toggleNotifPanel(wrap) {
  ...
  const body = items.length
    ? items.map((n, i) => `<button type="button" class="notif-item" data-route="${n.route || ''}" data-i="${i}" style="display:flex;align-items:flex-start;gap:10px;width:100%;text-align:${_rtl ? 'right' : 'left'};background:none;border:none;border-radius:10px;padding:9px 10px;cursor:pointer;color:var(--text)" onmouseover="this.style.background='var(--surface-2)'" onmouseout="this.style.background='none'">
...
function loginScreen() {
  ...
  card.innerHTML = `
    <div style="text-align:${getLang() === 'ar' ? 'left' : 'right'}"><button id="login-lang" class="btn ghost" style="padding:3px 10px;font-size:12px" title="English / العربية / Français">${({ en: 'العربية', ar: 'Français', fr: 'English' })[getLang()] || 'العربية'}</button></div>
  ...
  if (langToggle) langToggle.addEventListener('click', () => { setLang(({ en: 'ar', ar: 'fr', fr: 'en' })[getLang()] || 'en'); loginScreen(); });
...
const ROUTES = {
  ...
  coachsalary: { label: 'My Salary', icon: '💰', section: 'Main', coachOnly: true, hidden: true },   // v6.419 disabled per owner — coaches no longer see the salary page
...
const ROLE_ALLOWED = {
  ...
  coach: ['coachhome', 'coachattendance', 'schedule', 'campschedule', 'attendance', 'trials', 'advice', 'posts'],
...
function renderSidebar() {
  ...
  // Language toggle — cycles English → العربية → Français → English. Affects the member-facing
  // screens + RTL (Arabic only; French is LTR). The button shows the CURRENT language. (v6.437)
  const _langCycle = { en: 'ar', ar: 'fr', fr: 'en' };
  const _langLabel = { en: 'EN', ar: 'ع', fr: 'FR' };
  const langBtn = el('button', {
    id: 'quick-lang',
    title: 'English / العربية / Français',
    onclick: () => { setLang(_langCycle[getLang()] || 'en'); render(); },
  }, _langLabel[getLang()] || 'EN');
...
async function init() {
  ...
  async function showSessionResumePrompt() {
    ...
    wrap.innerHTML = `
      <div style="background:var(--surface,#fff);color:var(--text,#0f172a);max-width:400px;width:100%;border-radius:14px;padding:22px;box-shadow:0 20px 60px rgba(0,0,0,.35)">
        ...
        <div class="field" style="margin-bottom:6px">
          ...
          <input id="sr-pass" type="password" autocomplete="current-password" style="width:100%" />
        </div>
        <div id="sr-msg" style="font-size:12px;min-height:18px;color:var(--red,#dc2626)"></div>
        <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:10px">
          <button id="sr-later" class="btn ghost">${t('Later', 'لاحقاً')}</button>
    ...
    document.getElementById('sr-later').onclick = () => {
      close();
      toast(t('Your change is safe on this device and will be saved when you sign in.', 'تغييرك محفوظ على هذا الجهاز وسيُحفظ عند تسجيل الدخول.'), 'info');
    };
    const go = async () => {
      ...
      try {
        ...
        const r = await (window.Storage.retryNow ? window.Storage.retryNow() : Promise.resolve({ ok: true }));
        close();
        if (r && r.ok !== false) toast('✅ ' + t('Signed in — your work is saved to the cloud', 'تم الدخول — عملك محفوظ في السحابة'), 'success');
        else toast(t('Signed in — still saving, it will retry automatically', 'تم الدخول — ما زال الحفظ جارياً وسيُعاد تلقائياً'), 'info');
        try { render(); } catch (_) {}
      } catch (e) {
