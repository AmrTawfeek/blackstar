...
function salaryPayments(rec) {
  ...
  if (rec.paidDate != null) return [{ id: 'legacy', amount: (rec.snapshotNet != null ? Number(rec.snapshotNet) : 0), date: rec.paidDate, method: rec.payMethod || 'cash', _legacy: true }];
  return [];
}
function salaryPaidTotal(rec) { return salaryPayments(rec).reduce((s, p) => s + (Number(p.amount) || 0), 0); }
// The agreed payout: explicit override, else the legacy snapshot, else the computed net.
function salaryTarget(rec, netFallback) {
  if (rec && rec.target != null && rec.target !== '') return Number(rec.target);
  if (rec && rec.snapshotNet != null) return Number(rec.snapshotNet);
  return netFallback;
}
...
function computeMonthlyPay(coachId, monthKey, uptoDate) {
  ...
  const paidRemaining = Math.max(0, paidTarget - paidTotal);
  let paidStatus = 'pending';
  if (paidRecord) {
    if (paidTotal >= paidTarget - 0.005 && paidTotal > 0.005) paidStatus = 'paid';
