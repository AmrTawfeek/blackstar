...
const APP_VERSION = '6.395.0';   // 6.395.0 INSTALLMENTS: STOP DESTROYING THE PAYMENT LEDGER. Reported: any customer paying in installments ended up with inaccurate invoices, and installments were hard to manage. Review found three faults. (1) THE ACCURACY BUG - the Edit-invoice panel REPLACED inv.payments with a SINGLE row dated the invoice date whenever the collected figure changed. For a customer paying 300+300+300 across May/June/July that wiped the entire history in one click: individual installments gone, the who-took-it stamps gone, per-sport tags gone, and because cash-collected per month is summed from each row's OWN month, the money jumped into the invoice's month and two months of figures reported wrong. Worst of all it needed no admin input - merely LOWERING the invoice total triggered it through the paid<=total clamp. A correction now posts a dated adjustment row for the difference and leaves every installment intact, dated in the month of the money it corrects (not today) so revenue never shifts. A legacy invoice with no ledger still gets one seeded. (2) THE PAID FIGURE LIVES TWICE - the scalar amountPaid and the payments[] rows. recordPayment keeps them equal but several older paths set the scalar alone, and NOTHING detected the disagreement, so invoicePaid() (used by the invoice row, balance and Paid/Partial badge) could report 900 while invoicePaymentsSum() and the collected figures reported 600. New _icPaidLedgerIssue/findPaidLedgerDrift detect it and REPORT it for admin review - deliberately never auto-corrected, because choosing which number is true is a money decision for the owner, not the app. (3) THE PAY PANEL SHOWED NO HISTORY - staff taking installment 3 could not see 1 and 2 without leaving the screen. It now lists every installment with date, method, amount and who took it, with correction rows labelled as such rather than looking like customer payments. Tests: test-installments 28/28, control-verified (the old rewrite leaves one row and moves June's 300 into May). pages.js changed.
