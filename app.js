...
// ─── Idle auto-logout ────────────────────────────────────────────────
// Sign the user out after IDLE_LOGOUT_MS of no activity (protects a shared
// front-desk device left unattended). A short countdown warns first so nobody is
// logged out mid-task without notice — any click / keypress / mouse move cancels
// it. Pending cloud writes are flushed before signing out so no in-flight change
// is lost. Change IDLE_LOGOUT_MS below to adjust the timeout.
const IDLE_LOGOUT_MS = 5 * 60 * 1000;   // ← 5 minutes of inactivity
const IDLE_WARN_MS   = 30 * 1000;       // ← warn 30s before signing out
let _idleTimer = null, _idleWarnTimer = null, _idleWarnIv = null;
function _idleSignedIn() { return !!(typeof state !== 'undefined' && state && state.user); }
function _idleClearWarn() {
  if (_idleWarnIv) { clearInterval(_idleWarnIv); _idleWarnIv = null; }
...
function _idleWarn() {
  ...
  el.id = 'idle-warn';
  el.style.cssText = 'position:fixed;left:50%;top:20px;transform:translateX(-50%);z-index:100000;max-width:440px;width:calc(100% - 32px);background:#7f1d1d;color:#fff;border:1px solid rgba(255,255,255,.25);border-radius:12px;box-shadow:0 12px 40px rgba(0,0,0,.4);padding:14px 16px;display:flex;align-items:center;gap:12px;font-size:13.5px;line-height:1.45';
  el.innerHTML = `<div style="flex:1">🔒 <b>${t('Signing out for inactivity', 'تسجيل الخروج لعدم النشاط')}</b> — ${t('in', 'خلال')} <b id="idle-count">${secs}</b>s. ${t('Move the mouse or click to stay signed in.', 'حرّك الماوس أو انقر للبقاء.')}</div>
    <button id="idle-stay" style="background:#fff;color:#7f1d1d;border:none;border-radius:8px;padding:8px 12px;font-weight:700;cursor:pointer;white-space:nowrap">${t('Stay', 'البقاء')}</button>`;
  document.body.appendChild(el);
  try { el.querySelector('#idle-stay').addEventListener('click', _idleReset); } catch (_) {}
  _idleWarnIv = setInterval(() => {
    secs -= 1;
    const c = document.getElementById('idle-count'); if (c) c.textContent = String(Math.max(0, secs));
    if (secs <= 0 && _idleWarnIv) { clearInterval(_idleWarnIv); _idleWarnIv = null; }
  }, 1000);
}
function _idleReset() {
  clearTimeout(_idleTimer); clearTimeout(_idleWarnTimer);
  _idleClearWarn();
  if (!_idleSignedIn()) return;   // no timers on the login screen
  _idleWarnTimer = setTimeout(_idleWarn, Math.max(0, IDLE_LOGOUT_MS - IDLE_WARN_MS));
  _idleTimer = setTimeout(_idleExpire, IDLE_LOGOUT_MS);
}
window._idleReset = _idleReset;   // started after a successful login / on init when already signed in
(function _wireIdleActivity() {
  ...
  // Throttle high-frequency events so we don't reset the timers on every pixel of movement.
  let _last = 0;
...
const ROUTES = {
  ...
  monthlyreport: { label: 'Monthly Report', icon: '🗓', section: 'Insights', adminOnly: true, hidden: true },
  payanalysis:{ label: 'Payments Analysis', icon: '💳', section: 'Insights' },
  coachperf:  { label: 'Coach Performance', icon: '📊', section: 'Insights', hidden: true },
  ...
  transactions:{ label: 'Transactions', icon: '🧾', section: 'Insights', hidden: true },
  missinginvoices: { label: 'Missing Invoices', icon: '🧩', section: 'Insights', adminOnly: true, hidden: true },
  membercommission: { label: 'Member Commission', icon: '🧾', section: 'Insights', adminOnly: true },
  renewals:   { label: 'Renewals',   icon: '🔄', section: 'Insights', hidden: true },
  renewaldetail: { label: 'Renewal Potential', icon: '💰', section: 'Insights', hidden: true, adminOnly: true },
  attreport:  { label: 'Attendance Report', icon: '📋', section: 'Insights', hidden: true },
