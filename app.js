...
async function withCloudConfirm(opts) {
  ...
  // READ-BACK VERIFICATION (v6.332): for creates, don't trust the write ack alone — actually
  // RE-READ the record from the SERVER and confirm it exists before declaring success. opts.verify
  // = [{collection, id}, …]. If any is not retrievable, treat as a FAILURE (no false success).
  let verifyFailed = false;
  if (res && res.ok && Array.isArray(opts.verify) && opts.verify.length && window.Storage && typeof window.Storage.verifyDoc === 'function') {
    for (const v of opts.verify) {
      let exists = false;
      try { exists = await window.Storage.verifyDoc(v.collection, v.id); } catch (_) { exists = false; }
      if (!exists) { verifyFailed = true; break; }
    }
  }
  if (btn) { btn.disabled = false; if (btnHtml != null) btn.innerHTML = btnHtml; }
  if (res && res.ok && !verifyFailed) {
    if (typeof opts.onOk === 'function') { try { opts.onOk(res); } catch (_) {} }
    else if (opts.okMsg) { try { toast('✓ ' + opts.okMsg, 'success'); } catch (_) {} }
    return true;
  }
  if (verifyFailed) {
    try { toast('⚠ ' + t('Could NOT verify the record on the server — do not assume it saved. Retry.', 'تعذّر التأكد من حفظ السجل على الخادم — لا تفترض أنه حُفظ. أعد المحاولة.'), 'error'); } catch (_) {}
...
function showCloudUnavailableScreen() {
  try {
    ...
    d.innerHTML = '<div style="max-width:440px">'
      ...
      + '<div style="margin-top:14px;font-size:11px;color:#6b7280">Black Stars CRM · v' + (typeof APP_VERSION !== 'undefined' ? APP_VERSION : '') + '</div>'
      + '</div>';
...
async function load() {
  try {
    ...
    if (parsed && parsed.__cloudUnavailable) { window.__cloudUnavailable = true; showCloudUnavailableScreen(); return 'blocked'; }
...
async function init() {
  ...
  if (usedSaved === 'blocked') return;   // CLOUD-ONLY: cloud unreachable — blocking screen shown, do NOT start the app
  if (!usedSaved) {
