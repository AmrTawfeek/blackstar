...
const SEED_VERSION = '2026-06-08-v190-rentals-fixes';
...
function loginScreen() {
  ...
  const doLogin = async () => {
    ...
    const loginId = looksPhone ? phoneToMemberEmail(raw) : raw;
    const btn = $('#login-btn');
    btn.disabled = true;
    btn.textContent = 'Signing in...';
    try {
      const user = await window.Storage.signIn(loginId, p);
      ...
      // Still on the default password (mobile == password)? Force a change.
      const stillDefault = looksPhone && p.replace(/\D/g, '') === raw.replace(/\D/g, '');
      render();
...
function roleForEmail(email) {
  ...
  const fallback = (state.settings && state.settings.unmappedRole) || 'admin';
  return { role: hasAny ? fallback : 'admin', coachId: null, memberId: null };
}
...
function renderSidebar() {
  ...
  if (currentRole() !== 'admin') {
    ...
    const banner = el('div', {});
    banner.style.cssText = 'margin:8px 12px;padding:8px 10px;background:rgba(245,158,11,.14);border:1px solid rgba(245,158,11,.4);border-radius:8px;font-size:11px;display:flex;align-items:center;gap:8px';
    banner.innerHTML = `<span style="flex:1">${isAdminPreview ? '👁 Previewing as' : '🔒 Signed in as'} <b>${ROLE_LABELS[currentRole()] || currentRole()}</b></span>`;
    if (isAdminPreview) {
  ...
  for (const section of sections) {
    const entries = Object.entries(ROUTES).filter(([key, route]) => route.section === section && roleCanAccess(currentRole(), key) && (!route.memberOnly || currentRole() === 'student'));
