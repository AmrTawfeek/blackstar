...
const APP_VERSION = '6.239.1';   // multi-user backend + invoicing month fixes + inline payment re-dating; 6.239.1 fixes split-payment apply crash on invoice-less sport
...
// otherwise it bills in the invoice's month. This lets ONE invoice document span
// months — each sport's revenue counts in the month it was added — without ever
// splitting the invoice into a second document.
...
// No lineItems (rentals/sales/products) -> the whole invoice in its own month.
...
function lineCommissionEligibility(m, inv, li, uptoDate) {
  ...
  // Everyone else (Active, Frozen, …) → pro-rate by attendance.
