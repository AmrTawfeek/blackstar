...
function _scrollContainer() {
  return document.querySelector('.content') || document.querySelector('main') || document.scrollingElement || document.documentElement;
}
function navigate(route, opts) {
  ...
  if (!(opts && opts.isBack) && state.route && state.route !== route) {
    ...
    window._navStack.push({ route: state.route, scroll: sc ? sc.scrollTop : 0 });
  ...
  try {
    if (!(opts && opts.isBack && opts.fromPop)) {
      if (opts && opts.isBack) { /* our Back already handled; sync URL without new entry */ history.replaceState({ route }, '', '#' + route); }
  ...
  if (opts && typeof opts.scroll === 'number') {
    requestAnimationFrame(() => { const sc = _scrollContainer(); if (sc) sc.scrollTop = opts.scroll; });
  } else if (!(opts && opts.isBack)) {
    requestAnimationFrame(() => { const sc = _scrollContainer(); if (sc) sc.scrollTop = 0; });
  }
}
