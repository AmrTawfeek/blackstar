...
function detectDuplicateInvoices() {
  ...
  for (const inv of usable) {
    ...
    const key = (inv.customerId || memName) + '|' + cat + '|' + items + '|' + (inv.month || (inv.date || '').slice(0, 7)) + '|' + (inv.amount || 0);
  ...
  // Tier 2 — POSSIBLE (same customer+cat+items+amount, ≤7 days apart, not already exact)
