...
function mergeRemoteIntoState(remoteState) {
  ...
  for (const key of MERGE_COLLECTIONS) {
    ...
    const { merged, conflicts } = _mergeCollection(base[key] || [], localArr, remoteArr, key);
...
function renderSidebar() {
  ...
  // Navigation — ONE flat list, no category headers (owner request, v6.419). Items keep
  // the old section ORDER (Main → Membership → … → System) so related pages still sit
  // together, just without the collapsible group headings.
  ...
  for (const section of sections) {
    for (const [key, route] of Object.entries(ROUTES)) {
      ...
      flatEntries.push([key, route]);
    }
  }
  for (const [key, route] of flatEntries) {
