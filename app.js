...
function _dedupeSubsGuard() {
  ...
  for (const m of (state.members || [])) {
    ...
    // ── enrollments: no id, one-per-sport by design → collapse EXACT-content clones ──
    const enr = m && m.enrollments;
    if (Array.isArray(enr) && enr.length >= 2) {
      const seen = new Set(); const out = []; let hit = false;
      for (const e of enr) { const k = _enrKey(e); if (seen.has(k)) { enrCollapsed++; hit = true; continue; } seen.add(k); out.push(e); }
