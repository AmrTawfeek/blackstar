...
const APP_VERSION = '6.244.0';   // multi-user backend + invoicing fixes; 6.242.0 revenue by SPORT START MONTH; 6.243.x Fix-invoice-totals + inline amount + Refund + Cash-Collected + per-month SHARE; 6.244.0 PER-PAYMENT SPORT TAG — each installment says which sport it pays for, so per-month Paid/Due is exact (a payment tagged to a later-month sport counts in that month even if paid earlier); untagged falls back to earliest-month-first
  ...
  for (const id of allIds) {
    ...
    if (inL && inR) {
      ...
      if (lChanged && rChanged && _stableStr(l) !== _stableStr(r)) {
        out.push(l); conflicts++;            // true conflict → keep local, warn
...
function save() {
  // Stale-version guard: if this browser is running OLD cached code (the cloud
  // has a newer app version), block the save so it can't overwrite newer data
  // with stale data. Tell the user to refresh.
  if (_staleVersion) {
    try { if (typeof showStaleVersionBanner === 'function') showStaleVersionBanner(); } catch (_) {}
  ...
  try {
    ...
    stateToSave = { ...persistable, __schema: SCHEMA_VERSION, __appVersion: APP_VERSION };
...
function recordPayment(inv, opts) {
  ...
  const row = { date, month: String(date).slice(0, 7), amount: amt, method: opts.method || 'cash' };
...
function lineSportStartMonth(li, inv) {
  ...
  // Prefer the subscription tied to THIS invoice; else any subscription for the sport.
  const sub = mem.subscriptions.find(s => s && s.invoiceNumber === inv.ref && (s.activity || '') === sport)
           || mem.subscriptions.find(s => s && (s.activity || '') === sport);
...
function billedInMonth(ym, pred) {
  ...
  for (const i of (state.invoices || [])) {
    ...
    if (sh) t += (Number(i.amount) || 0) * sh;
  }
  return t;
}
// Collected toward billing = paid capped at the invoice amount (= amount − balance),
// so an overpaid/credit invoice can never make "collected" exceed "billed" and the
// identity billed = collected + due always holds. Raw cash (incl. any overpayment)
// is tracked separately via cashInMonth / the payments log.
function collectedInMonth(ym) {
  let t = 0;
  for (const i of (state.invoices || [])) {
    if (!i || i.deleted) continue;
    const sh = invoiceMonthShare(i, ym);
    if (sh) t += Math.min(invoicePaid(i), Number(i.amount) || 0) * sh;
  }
  return t;
}
...
function invoicePaidInMonth(inv, ym) {
  ...
  if (!pays.length) {   // legacy invoice, no ledger → its paid sits in its own month
    return (invoiceBillMonth(inv) === ym) ? Math.min(invoicePaid(inv), Number(inv.amount) || 0) : 0;
  }
...
function billedInPeriod(monthPred) {
  ...
  for (const i of (state.invoices || [])) {
    if (!i || i.deleted) continue;
    for (const [mo, sh] of invoiceMonthShares(i)) if (monthPred(mo)) t += (Number(i.amount) || 0) * sh;
  }
  return t;
}
function billedByCategoryInPeriod(monthPred) {
  ...
  for (const i of (state.invoices || [])) {
    ...
    for (const [mo, sh] of invoiceMonthShares(i)) if (monthPred(mo)) out[cat] = (out[cat] || 0) + (Number(i.amount) || 0) * sh;
  }
  return out;
}
...
function computeAttendanceCommission(coachId, monthKey, uptoDate) {
  ...
  for (const inv of state.invoices) {
    ...
    for (const li of lineItems) {
      ...
      // A FROZEN membership is paused, not finished: pay the classes the member has
      // ACTUALLY attended (handled by the attended-per-month logic below), but never
      // pay out the remainder — it stays PENDING until they return, or the freeze
      // ends and they expire. Only a genuinely EXPIRED membership trues up to full.
...
function computeMonthlyPay(coachId, monthKey, uptoDate) {
  ...
  const expensePaid = (state.expenses || [])
    .filter(e => !e.deleted && isSalaryCategory(e.category) && String(e.coachId) === String(coachId)
...
const ROUTES = {
  ...
  bankaccount: { label: 'Bank Account', icon: '🏦', section: 'Finance', adminOnly: true },
  reconciliation: { label: 'Reconciliation', icon: '⚖️', section: 'Finance', adminOnly: true },
  ...
  dashboardkpi: { label: 'Owner Dashboard', icon: '📊', section: 'Insights', adminOnly: true },
  monthlyreport: { label: 'Monthly Report', icon: '🗓', section: 'Insights', adminOnly: true },
  payanalysis:{ label: 'Payments Analysis', icon: '💳', section: 'Insights' },
  coachperf:  { label: 'Coach Performance', icon: '📊', section: 'Insights' },
  clubrevenue:{ label: 'Club Revenue Summary', icon: '💼', section: 'Insights' },
  moneyflow:  { label: 'Financial Overview', icon: '💰', section: 'Insights', adminOnly: true },
  transactions:{ label: 'Transactions', icon: '🧾', section: 'Insights' },
  missinginvoices: { label: 'Missing Invoices', icon: '🧩', section: 'Insights', adminOnly: true },
  membercommission: { label: 'Member Commission', icon: '🧾', section: 'Insights', adminOnly: true },
...
const ROLE_ALLOWED = {
  ...
  coach: ['coachhome', 'schedule', 'campschedule', 'attendance', 'trials', 'advice', 'posts'],
...
async function init() {
  ...
  window.__onCloudSaveStatus = (() => {
    ...
    const pill = () => {
      ...
      if (!p) {
        ...
        p.style.cssText = 'position:fixed;right:14px;bottom:14px;z-index:9998;font-size:12px;font-weight:700;padding:6px 12px;border-radius:999px;box-shadow:0 4px 14px rgba(0,0,0,.28);pointer-events:none;transition:opacity .25s;font-family:system-ui,sans-serif';
    ...
    return (s) => {
      try {
        ...
        if (s.phase === 'saving') { p.style.background = '#f59e0b'; p.style.color = '#fff'; p.textContent = '☁️ ' + t('Saving…', 'جارٍ الحفظ…'); }
        else if (s.phase === 'saved') {
          p.style.background = '#16a34a'; p.style.color = '#fff';
          p.textContent = '✅ ' + t('Saved to cloud', 'حُفظ في السحابة') + ' · ' + new Date(s.at || Date.now()).toLocaleTimeString();
          hideTimer = setTimeout(() => { p.style.opacity = '0'; }, 4000);
        } else if (s.phase === 'error') { p.style.background = '#b91c1c'; p.style.color = '#fff'; p.textContent = '⚠️ ' + t('Not saved — retrying', 'لم يُحفظ — إعادة المحاولة'); }
      } catch (_) {}
    };
  })();
  ...
  // If Firestore writes start failing (e.g. quota/connection), tell the user clearly
  // instead of failing silently. Their data is still kept locally as a safety net.
  window.__onCloudSaveError = (() => {
    let shown = 0;
    return (err) => {
      const now = Date.now();
      if (now - shown < 20000) return;   // don't spam — at most once per 20s
      shown = now;
      const code = (err && err.code) || '';
      const isQuota = /resource-exhausted|exhausted|quota/i.test(code + ' ' + (err && err.message || ''));
      try {
        if (document.getElementById('cloud-save-error-note')) return;
        const bar = document.createElement('div');
        bar.id = 'cloud-save-error-note';
        bar.style.cssText = 'position:fixed;left:50%;transform:translateX(-50%);bottom:18px;z-index:9999;max-width:600px;width:calc(100% - 32px);background:#7f1d1d;color:#fff;border:1px solid rgba(255,255,255,.2);border-radius:12px;box-shadow:0 8px 30px rgba(0,0,0,.35);padding:12px 14px;display:flex;align-items:center;gap:12px;font-size:13px;line-height:1.4';
        const msg = isQuota
          ? `⚠️ <b>${t('Cloud is busy', 'الخادم مشغول')}</b> — ${t('too many rapid saves. Your data is safe locally and will sync automatically in a moment. Avoid rapid repeated edits.', 'عدد كبير من عمليات الحفظ السريعة. بياناتك محفوظة محلياً وستتم المزامنة تلقائياً بعد لحظات. تجنّب التعديلات السريعة المتكررة.')}`
          : `⚠️ <b>${t('Cloud save delayed', 'تأخّر الحفظ السحابي')}</b> — ${t('your data is saved locally and will sync when the connection recovers.', 'بياناتك محفوظة محلياً وستتم المزامنة عند عودة الاتصال.')}`;
        bar.innerHTML = `<div style="flex:1">${msg}</div>
          <button id="cloud-save-error-dismiss" style="background:transparent;color:#fecaca;border:none;padding:8px 6px;cursor:pointer;white-space:nowrap">${t('OK', 'حسناً')}</button>`;
        document.body.appendChild(bar);
        document.getElementById('cloud-save-error-dismiss').addEventListener('click', () => bar.remove());
        setTimeout(() => { try { bar.remove(); } catch (_) {} }, 10000);
      } catch (_) {}
    };
  })();
