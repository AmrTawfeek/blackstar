...
const SEED_VERSION = '2026-06-07-v185-wipe-guard';
...
const ROLE_ALLOWED = {
  admin: null,
  coach: ['dashboard', 'schedule', 'campschedule', 'attendance', 'members', 'trials', 'salaries'],
  student: ['dashboard', 'schedule', 'campschedule', 'expiring'],
};
...
function renderSidebar() {
  ...
  // Role preview banner (only when not Admin) — quick exit back to full access.
  if (currentRole() !== 'admin') {
    ...
    banner.innerHTML = `<span style="flex:1">👁 Previewing as <b>${ROLE_LABELS[currentRole()] || currentRole()}</b></span>`;
  ...
  for (const section of sections) {
    const entries = Object.entries(ROUTES).filter(([key, route]) => route.section === section && roleCanAccess(currentRole(), key));
