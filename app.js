...
function _elKey(e) {
  ...
  return 's:' + _stableStr(e);   // id-less element → identity by content (dedupes identical rows)
}
...
function _mergeArrayById(baseArr, localArr, remoteArr) {
  ...
  for (const k of order) {
    ...
    if (inL && inR) {
      ...
      const rCh = !inB || _stableStr(r) !== _stableStr(b);
      if (inB && !rCh) { /* removed locally, untouched remotely → drop */ } else out.push(r);
    }
  }
  return out;
}
...
function _mergeRecord(base, local, remote) {
  ...
  for (const k of keys) {
    ...
    if (Array.isArray(lv) || Array.isArray(rv)) { out[k] = _mergeArrayById(bv, lv, rv); continue; }
    if (_isPlainObj(lv) && _isPlainObj(rv)) { out[k] = _mergeRecord(bv, lv, rv); continue; }
  ...
  for (const id of allIds) {
    ...
    if (inL && inR) {
      ...
      if (lChanged && rChanged && _stableStr(l) !== _stableStr(r)) {
        ...
        out.push(_mergeRecord(b, l, r)); conflicts++;
...
function save() {
  ...
  if (_staleVersion) {
    ...
    return false;   // BLOCKED — nothing persisted; saveConfirmed() must report this as a failure
  }
