...
const APP_VERSION = '6.252.0';   // 6.251.0 = permissions batch. 6.252.0 = REQUIREMENTS BATCH 2 (audit + last-updated): (#8) Audit Trail is now ADMIN-ONLY, enriched (full name, role, module, record id/name, previous→new value), with filters (date range/user/module/action) + search (member/mobile/invoice#/username) + Excel/PDF/CSV export; attendance & freeze are now logged; the audit log is IMMUTABLE (append-only, no delete; firestore.rules deny update/delete on auditLog). (#5) records are stamped with updatedBy/updatedByName/updatedAt (+createdBy/At) — members, invoices, payments, attendance, freeze, users — shown as "Last updated by X · date" on the member profile and user lookup
...
function invoiceBilledInMonth(inv, ym) { return (Number(inv && inv.amount) || 0) * invoiceMonthShare(inv, ym); }
...
function settleCoachPendingCommission(coachId, monthKey) {
  ...
  for (const inv of (state.invoices || [])) {
    ...
    if (!mem || mem.deleted) continue;
    const lines = Array.isArray(inv.lineItems) && inv.lineItems.length
      ? inv.lineItems : [{ sport: inv.sport, coachId: inv.coachId, price: inv.amount || 0 }];
    for (const li of lines) {
      if (li.coachId !== coachId) continue;
      if (li.sport === SUMMER_CAMP) continue;
      const sub = findSubForLine(mem, inv, li);
...
const ROLE_ALLOWED = {
  ...
  receptionist: [
    ...
    // (least-privilege, req #9). The WhatsApp onboarding screen is added below.
  ],
};
