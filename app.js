...
const SEED_VERSION = '2026-06-04-v139-phone-search';
  ...
  for (const inv of state.invoices) {
    ...
    for (const li of lineItems) {
      ...
      if (isSwitch || totalClasses <= 0) {
        if (inv.month === monthKey && (!uptoDate || (inv.date && inv.date <= uptoDate))) {
          base += fee;
      ...
      const attMonth = attendedYInMonth(mem, li.sport, monthKey, sub.start, sub.end, uptoDate);
      if (attMonth > 0) {                                  // (1) classes attended this month
      ...
      const ended = !frozen && sub.end && (uptoDate ? sub.end <= uptoDate : sub.end < TODAY);
      const attendedAll = attendedYForSub(mem, sub, uptoDate);
      const remaining = Math.max(0, totalClasses - attendedAll);
      if (endMonth === monthKey && ended && remaining > 0) {  // (2) expiry true-up
      ...
      if (!ended && remaining > 0) {                          // (3) pending (still active)
...
function computeMonthlyPay(coachId, monthKey, uptoDate) {
  ...
  if (basis === 'attendance') {
    ...
    // Payment basis (default, unchanged): the whole fee counts in the invoice's month.
    for (const inv of state.invoices) {
      if (monthKey && inv.month !== monthKey) continue;
      if (uptoDate && (!inv.date || inv.date > uptoDate)) continue;   // settlement cap
      const cat = inv.category || 'Membership';
