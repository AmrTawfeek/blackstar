...
const SEED_VERSION = '2026-06-08-v263-clubrevenue-receptionist-sortable'
...
function coachEarnings(coach, monthKey) {
  ...
  for (const inv of (state.invoices || [])) {
    if (inv.coachId !== coach.id) continue;
    if (monthKey && inv.month !== monthKey) continue;
    const amt = inv.amount || 0;
    revenue += amt;
    if (inv.customerId) {
      const mem = state.members.find(x => x.id === inv.customerId);
      if (mem) { studentSet.add(mem.id); if (isActiveStatus(mem)) commissionBase += amt; }
    }
  }
...
function syncSubToEnrollment(sub, e, member, invoices) {
  ...
  const eValidity = e.sport === SUMMER_CAMP ? (e.classes || DEFAULT_VALIDITY) : (e.validity || DEFAULT_VALIDITY);
...
// Returns groups (length>1) of likely-duplicate Membership invoices: same member,
// sport, month and amount. Switch credits and negatives are ignored.
function detectDuplicateInvoices() {
  ...
  for (const inv of state.invoices) {
    ...
    const memName = mem ? mem.name : (inv.customerName || '—');
    const sport = inv.sport || (Array.isArray(inv.lineItems) ? inv.lineItems.map(l => l.sport).filter(Boolean).sort().join('+') : '') || '—';
    const key = (inv.customerId || memName) + '|' + sport + '|' + (inv.month || '') + '|' + (inv.amount || 0);
    (groups[key] = groups[key] || []).push({ inv, memName, sport });
  }
...
// Self-service freeze allowance: 5 days per 30 days of validity (30d→5, 60d→10,
// 90d→15 …), tracked per current membership cycle (renewing resets it). A single
// freeze request is also capped at 7 days ("up to one week at a time").
const FREEZE_PER_REQUEST_CAP = 7;
function freezeAllowance(m) {
  const validityDays = memberValidityDays(m);
  const allowanceDays = Math.floor(validityDays / 30) * 5;
  const cycleStart = m?.startDate || m?.firstRegistration || '0000-00-00';
  let usedDays = 0;
  for (const f of (m?.freezes || [])) {
    if ((f.start || '') >= cycleStart) usedDays += (parseInt(f.days) || 0);
  }
  const remainingDays = Math.max(0, allowanceDays - usedDays);
  return { validityDays, allowanceDays, usedDays, remainingDays, cycleStart, perRequestCap: FREEZE_PER_REQUEST_CAP };
}
...
const ROUTES = {
  ...
  campmembers: { label: 'Camp Members', icon: '🚌', section: 'Summer Camp' },
...
const ROLE_ALLOWED = {
  ...
  // Receptionist: front-desk + read-only finance. Can manage members, families,
  // trials, attendance, invoices, schedule, summer camp; can READ revenue, profit,
  // salaries via Reports / Club Revenue Summary / Salaries (the pages themselves
  // are read-only for them). NO access to coach pay edits, system settings,
  // accounts/roles, danger zone, backup, or audit log.
  receptionist: [
    'dashboard', 'members', 'families', 'history', 'expiring', 'trials', 'reminders',
    'schedule', 'attendance', 'rentals', 'advice',
    'campschedule', 'campmembers', 'campdrivers', 'camproutes',
    'invoices', 'salaries', 'products', 'sales',
    'reports', 'coachperf', 'renewals', 'renewaldetail', 'attreport', 'clubrevenue',
    'coaches',
  ],
};
...
async function init() {
  ...
  if (window.Storage.isCloud()) {
    window.Storage.onRemoteUpdate(remoteState => {
      ...
      // Apply remote DATA only — keep THIS device's open page, identity and preview role.
      const savedUser = state.user, savedRoute = state.route, savedSession = state.session;
