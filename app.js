...
const SEED_VERSION = '2026-06-08-v206-email-username-mobile-pw';
  ...
  const doLogin = async () => {
    ...
    try {
      ...
      if (looksPhone) {
        ...
        const emails = digitForms.map(x => x + '@' + MEMBER_EMAIL_DOMAIN);
...
const MEMBER_EMAIL_DOMAIN = 'members.blackstars.qa';
...
function isMemberEmail(email) {
  return typeof email === 'string' && email.toLowerCase().endsWith('@' + MEMBER_EMAIL_DOMAIN);
}
