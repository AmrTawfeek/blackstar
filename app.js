...
function render() {
  ...
  try {
    ...
    if (tb && !tb.querySelector('.cloud-refresh-btn')) {
      const rb = document.createElement('button');
      rb.className = 'btn ghost cloud-refresh-btn';
      rb.type = 'button';
      rb.title = t('Refresh data from cloud (no page reload)', 'تحديث البيانات من السحابة (بدون إعادة تحميل)');
      rb.setAttribute('aria-label', t('Refresh from cloud', 'تحديث من السحابة'));
      rb.style.cssText = 'margin-inline-start:auto;padding:8px 12px;font-size:16px;line-height:1;flex:0 0 auto';
      rb.innerHTML = '🔄';
      rb.addEventListener('click', () => window.refreshFromCloud(rb));
      tb.appendChild(rb);
      tb.style.display = tb.style.display || 'flex';
      tb.style.alignItems = tb.style.alignItems || 'center';
    }
  } catch (_) {}
