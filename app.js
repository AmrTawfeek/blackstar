...
const APP_VERSION = '6.380.0';   // 6.380.0 MONEY-CRITICAL: DELETED invoices no longer pay attendance commission (computeAttendanceCommission was missing the inv.deleted guard the by-payment path already had) — this is why soft-deleted 'Test' invoices + duplicates you'd removed kept showing on salary reports and over-crediting coaches. Against the owner's real backup this removed phantom base: Abdel Salam -1775 (~533 pay), Mostafa -417 (~125), Jennifer -366 (~110). Now 'Remove duplicates' (soft delete) actually removes from pay, and the DUP-EXCLUDED warnings from deleted-vs-live pairs disappear. Also: (2) detailed salary report groups members by SPORT with per-sport subtotals when a coach teaches >1 sport (# index continues across groups); (3) Expiring-screen WhatsApp reminder uses the 'completed' (congratulatory, come-renew) message for members who FINISHED their classes (membersReadyToRenew), not the plain expired/expiring wording. Tests: test-deleted-invoice-commission 5/5 (control-verified + run against the real backup), test-salary-group-and-completed-msg 9/9, full regression green, logic-tests 1226/0. app.js + pages.js changed.
