...
function _elTombstone(key) { if (_isIdKey(key)) _elTomb.set(key, Date.now() + _EL_TOMB_MS); }
...
if (typeof window !== 'undefined') { window._tombstoneEl = e => { try { _elTombstone(_elKey(e)); } catch (_) {} }; }
...
function _mergeArrayById(baseArr, localArr, remoteArr) {
  ...
  for (const k of order) {
    ...
    if (inL && inR) {
      ...
      // explicitly tombstoned by a delete (honour it even if it isn't in the base).
      if ((inB && !rCh) || _elIsTombstoned(k)) { /* deleted → do not resurrect */ } else out.push(r);
    }
  }
  return out;
}
...
function _enrKey(e) {
  ...
  const IGN = { _originalSport: 1, _paid: 1, _attended: 1 };
