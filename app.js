...
function splitSiblingPayment(members, familyTotal) {
  const sibs = (members || []).filter(Boolean);
  const n = sibs.length;
...
function findMisdatedInvoices(minGapDays = 3) {
  ...
  for (const inv of (state.invoices || [])) {
    ...
    // Earliest real start: from enrollments (per-sport start), else member start.
...
function mergeMemberInvoices(memberId) {
  ...
  keep.amount = (keep.lineItems || []).reduce((s, li) => s + (Number(li.price) || 0), 0);
