...
const SEED_VERSION = '2026-06-08-v208-member-domain-blackstars-com';
...
let state = {
  ...
  settings: {
    expiringSoonDays: 3,
    lowStockThreshold: 3,
    commissionBasis: 'payment',   // 'payment' (fee counts in payment month) | 'attendance' (per class attended)
...
async function load() {
  try {
    ...
    if (state.settings.commissionBasis !== 'attendance') state.settings.commissionBasis = 'payment';
...
function fuzzyMatch(text, query) {
  ...
  const thr = query.length <= 3 ? 1 : query.length <= 6 ? 2 : 3;
  if (levenshtein(text, query) <= thr) return true;
  for (const w of text.split(/\s+/)) {
    if (!w) continue;
    if (w.startsWith(query)) return true;
    if (levenshtein(w, query) <= thr) return true;
  }
  return false;
}
...
function coachTeachesSport(coach, sport) {
  ...
  if (!isBookableSport(sport)) return true;        // unknown / camp activity → no constraint
...
function loginScreen() {
  ...
  const userLabel = isCloud ? 'Email or mobile number' : 'Username';
  ...
  const hint = isCloud
    ? 'Staff: use your email + password. Members: use your mobile number (password is your mobile number the first time).'
...
const ROUTES = {
  ...
  campschedule: { label: 'Summer Camp', icon: '☀️', section: 'Main' },
  ...
  users:      { label: 'Users & Roles', icon: '🔐', section: 'Settings', adminOnly: true },
  preferences:{ label: 'Preferences', icon: '🎛', section: 'Settings', adminOnly: true },
  club:       { label: 'Club Setup',  icon: '🏷', section: 'Settings', adminOnly: true },
  databackup: { label: 'Data & Backup', icon: '💾', section: 'Settings', adminOnly: true },
...
const ROLE_ALLOWED = {
  ...
  coach: ['schedule', 'campschedule', 'attendance', 'trials'],
  // Students/members: their own membership + the class timetable. No other members.
  student: ['mymembership', 'schedule', 'campschedule'],
};
...
function renderSidebar() {
  ...
  // Collapse / expand toggle (desktop). Persists across sessions.
  const collapseBtn = el('button', {
    ...
    onclick: () => {
      ...
      collapseBtn.innerHTML = isNow ? '»' : '«';
    },
  }, sb.classList.contains('sidebar-collapsed') ? '»' : '«');
  brand.append(collapseBtn);

  // Logged-in user shown at the TOP of the sidebar (name + role).
  const topUser = el('div', { className: 'sidebar-userchip' });
  const _topName = (state.user && state.user.name) || 'User';
  const _topInit = (_topName.trim().split(/\s+/).map(w => w[0]).slice(0, 2).join('') || 'U').toUpperCase();
  const _topRole = ROLE_LABELS[accountRole()] || 'Administrator';
  topUser.innerHTML = `<div class="avatar" style="width:28px;height:28px;font-size:11px;flex-shrink:0">${escapeHtml(_topInit)}</div>
    <div style="flex:1;min-width:0" class="sidebar-userchip-text">
      <div style="font-size:12px;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${escapeHtml(_topName)}</div>
      <div style="font-size:10px;color:var(--text-mute)">${escapeHtml(_topRole)}</div>
    </div>`;
  sb.append(topUser);
  ...
  if (currentRole() !== 'admin') {
    ...
    banner.innerHTML = `<span style="flex:1">${isAdminPreview ? '👁 Previewing as' : '🔒 Signed in as'} <b>${escapeHtml(who)}</b> <span style="opacity:.7">· ${ROLE_LABELS[currentRole()] || currentRole()}</span></span>`;
    if (isAdminPreview) {
  ...
  const sections = ['Main','Finance','Insights','System','Settings'];
  for (const section of sections) {
    const entries = Object.entries(ROUTES).filter(([key, route]) => route.section === section && !route.hidden && roleCanAccess(currentRole(), key) && (!route.memberOnly || currentRole() === 'student') && (!route.adminOnly || currentRole() === 'admin'));
  ...
  const _roleLabel = ROLE_LABELS[accountRole()] || 'Administrator';
  ...
  footer.innerHTML = `
    ...
    ${_isAdmin ? `<button class="btn ghost sm full" id="sidebar-cmdk" style="margin-bottom:6px;display:flex;align-items:center;justify-content:flex-start;gap:8px;text-align:left" title="Quick search (Ctrl+K / ⌘K)"><span style="width:18px;text-align:center;flex-shrink:0">🔎</span><span style="flex:1;min-width:0">Quick search</span><span style="margin-left:auto;font-family:'JetBrains Mono',monospace;font-size:9px;padding:1px 4px;background:var(--surface);border:1px solid var(--border);border-radius:4px;flex-shrink:0">⌘K</span></button>` : ''}
    ${_isAdmin ? `<button class="btn ghost sm full" id="sidebar-backup" style="margin-bottom:6px;display:flex;align-items:center;justify-content:flex-start;gap:8px;text-align:left" title="Download a full JSON backup of your data"><span style="width:18px;text-align:center;flex-shrink:0">💾</span><span style="flex:1;min-width:0">Quick backup</span></button>` : ''}
    ${_isCloud ? `<button class="btn ghost sm full" id="sidebar-changepw" style="margin-bottom:6px;display:flex;align-items:center;justify-content:flex-start;gap:8px;text-align:left" title="Change your sign-in password"><span style="width:18px;text-align:center;flex-shrink:0">🔐</span><span style="flex:1;min-width:0">Change password</span></button>` : ''}
