...
async function init() {
  ...
  if (window.Storage.isCloud()) {
    window.Storage.onRemoteUpdate(remoteState => {
      ...
      state.user = savedUser; state.route = savedRoute; state.session = savedSession;
      // CRITICAL: if the merge changed nothing (e.g. the echo of our own save, or
      // identical data), do NOT save or re-render. Saving here would re-trigger the
      // remote listener and create an endless refresh loop.
      if (!result.changed) return;
      try { save(); } catch (_) {}
      render();
      if (result.conflicts > 0 && typeof toast === 'function') {
        toast(`⚠ ${result.conflicts} record${result.conflicts === 1 ? '' : 's'} edited on another device at the same time — your version was kept. Please double-check ${result.conflicts === 1 ? 'it' : 'them'}.`, 'error');
      }
    });
  }
