...
async function init() {
  ...
  if (window.Storage.isCloud()) {
    window.Storage.onRemoteUpdate(remoteState => {
      ...
      const savedUser = state.user, savedRoute = state.route, savedSession = state.session;
      let conflicts = 0;
      try { conflicts = mergeRemoteIntoState(remoteState); }
      catch (e) {
        console.warn('[sync] merge failed, keeping local data:', e);
        return;   // on any merge error, keep local data untouched (safest)
      }
      state.user = savedUser; state.route = savedRoute; state.session = savedSession;
      // Persist the reconciled result so it becomes the new shared truth.
      try { save(); } catch (_) {}
      render();
      if (conflicts > 0 && typeof toast === 'function') {
        toast(`⚠ ${conflicts} record${conflicts === 1 ? '' : 's'} edited on another device at the same time — your version was kept. Please double-check ${conflicts === 1 ? 'it' : 'them'}.`, 'error');
      }
    });
  }
