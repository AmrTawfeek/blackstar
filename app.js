...
const MERGE_COLLECTIONS = [
  ...
  'families', 'notes', 'cashCounts', 'swimGroups',
];
...
function billedInMonth(ym)    { return monthInvoices(ym).reduce((s, i) => s + (Number(i.amount) || 0), 0); }
function collectedInMonth(ym) { return monthInvoices(ym).reduce((s, i) => s + invoicePaid(i), 0); }
function dueInMonth(ym)       { return monthInvoices(ym).reduce((s, i) => s + invoiceBalance(i), 0); }
...
function lineCommissionEligibility(m, inv, li, uptoDate) {
  ...
  if (m) {
    ...
    if (sub) {
      ...
      attended = attendedClassesFor(m, li && li.sport);
    }
  }
  // COMPLETED → full fee.
  if (status === 'Completed') {
