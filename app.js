...
function computeMemberCommissions(ym) {
  ...
  const invs = all
    ...
    : monthInvoices(ym).filter(i => (i.category || 'Membership') === 'Membership');
  const rows = [];
  for (const inv of invs) {
    ...
    for (const li of lineItems) {
      ...
      rows.push({
        invoiceId: inv.id, ref: inv.ref || '', month: invoiceBillMonth(inv),
...
function computeMonthlyPay(coachId, monthKey, uptoDate) {
  ...
  if (basis === 'attendance') {
    ...
    for (const inv of state.invoices) {
      ...
      } else if (monthKey && invoiceBillMonth(inv) !== monthKey) {
