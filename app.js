...
function syncSubToEnrollment(sub, e, member, invoices) {
  ...
  if (eStart) {
    ...
    sub.end = (e.sport === SUMMER_CAMP && typeof campEndDate === 'function') ? campEndDate(eStart, eValidity) : addDays(eStart, eValidity);
  }
}
...
function deriveMemberDates(enrollments, firstRegInput) {
  ...
  const ends = list.map(e => {
    ...
    return (e.sport === SUMMER_CAMP && typeof campEndDate === 'function') ? campEndDate(st, v) : addDays(st, v);
  }).filter(Boolean).sort();
  return {
    startDate: minStart,
...
const ROUTES = {
  ...
  completed:  { label: 'Ready to Renew', icon: '✅', section: 'Membership', badge: () => (typeof completedRenewalCount === 'function' ? completedRenewalCount() : 0) },
...
function recentSearchChipsHtml(key, containerId) {
  const items = recentSearches(key).slice(0, RECENT_SEARCH_MAX);
...
async function init() {
  ...
  if (window.Storage.isCloud()) {
    ...
    try {
      ...
      // NOT 'mousemove': the pointer drifts constantly while you simply read the screen, which
      // kept the app permanently "busy" — so a remote change could never be applied quietly and
      // the "Newer data is available" bar showed instead. Deliberate actions only. (v6.341)
    ...
    const TOKEN_REFRESH_MS = 30 * 60 * 1000;   // 30 min — half the token lifetime, ample margin
    const _keepAuthAlive = () => { try { if (window.Storage && window.Storage.refreshAuth) window.Storage.refreshAuth(); } catch (_) {} };
    setInterval(_keepAuthAlive, TOKEN_REFRESH_MS);
    document.addEventListener('visibilitychange', () => { if (document.visibilityState === 'visible') _keepAuthAlive(); });
    window.addEventListener('online', _keepAuthAlive);
    window.addEventListener('focus', _keepAuthAlive);
  }
