...
function renderSidebar() {
  ...
  // Show a compact 🌐 + 2-letter code as the selected value (fits the narrow sidebar header and mobile
  // without clipping); the full language names appear when the dropdown is opened. (v6.446 UI fix)
  langBtn.innerHTML = [['en', 'English', 'EN'], ['ar', 'العربية', 'ع'], ['fr', 'Français', 'FR']]
    .map(([code, name]) => `<option value="${code}"${getLang() === code ? ' selected' : ''}>🌐 ${name}</option>`).join('');
  langBtn.addEventListener('change', () => { setLang(langBtn.value); render(); });
  langBtn.style.cssText = 'flex:0 0 auto;min-width:56px;max-width:120px;box-sizing:border-box;background:var(--surface-2);border:1px solid var(--border);cursor:pointer;font-size:12px;font-weight:600;padding:6px 6px;border-radius:8px;color:var(--text);margin-left:4px';
