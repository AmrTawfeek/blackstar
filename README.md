# Black Stars CRM
Version 6.238.0 - Added a safe Payments panel (editInvoicePayments). Opens from the Invoices list, the per-customer invoice history, and the Edit-pricing group header ('payments' link). It lists every payment row and lets you change a payment's DATE and METHOD, remove a row, or add a new dated payment. Editing date/method never changes an amount, and amountPaid is always recomputed as the exact sum of the rows - so it answers the 'change a payment date' need and also lets you clean up bad rows yourself, without any path that re-derives or re-splits (no corruption can return). This is the supported way to re-date an existing payment (the Edit-pricing date box only applies to newly collected money). Gates green (1089 tests, 35/35 render).
Version 6.237.0 - Rebuilt the payment model on the safe append-only ledger (model B), removing the per-sport re-splitting that corrupted payment history. New recordPayment(inv,{date,amount,method}) is now the ONLY way to add money: it appends one immutable, 2dp-rounded, finite-checked row and recomputes paid as the pure sum - existing rows are never rewritten, re-split, or re-derived, so the class of bug that produced the impossible +/- rows is architecturally impossible. Edit pricing was rewritten: per-sport Price/Discount stays, but the per-sport Paid-so-far field (the re-splitting source) is gone; instead each invoice has its own dated Record-payment box, and a newly added sport gets billMonth = its start month (so cross-month revenue lands in the right month). Payments are never tagged to a sport, so switching a sport never has to re-point money - line items + an explicit credit handle the switch, and revenue-by-sport still reads line prices/bill-months. Proven by tests: append-only + no-mutation + exact-sum through a full enroll/pay/add/SWITCH/pay lifecycle, plus a dataset invariant that every payments ledger sums to amountPaid. No existing data was modified. 1089 tests pass.
Version 6.236.0 - Temporarily removed the per-payment 'Payments received' history table from the printed invoice, per request, until per-payment tracking is stable. The invoice still shows Paid and Balance due; only the per-payment breakdown (which could display historical bad rows from an older version) is hidden. No data was changed - this is a display-only change and the block is easy to switch back on later.
Version 6.235.0 - Payments Analysis is now monthly, like the other financial screens. Added a Month dropdown (defaulting to the current data month) and rebuilt the screen to work PER PAYMENT instead of per invoice: each installment is counted in the month and method it was actually received, so an invoice paid partly in June and partly in July is split correctly across both months (the old screen dated the whole paid amount to the latest payment date, mis-attributing installments). By-method totals, expenses and cash-in-hand all respect the selected month; 'All months' restores the all-time view, and the day/range filters still work within it. Validated on the 30-Jun backup: June shows cash 38,960 + card 57,695 + Fawran 3,175 across 196 payments.
Version 6.234.0 - Moved the Reports screen into the 'Main' menu section, directly under Dashboard (was under 'Insights'). Visibility is unchanged - it stays admin-only via the role whitelists (coaches, students and receptionists never had Reports in their allowed pages), so this is purely a menu reorganization.
Version 6.233.0 - Fixed screens defaulting to a FUTURE month. A single future-dated invoice (e.g. a July renewal dated 1 Jul, created in June) was making latestDataMonth() return July, so the Salaries, Reports, and Attendance screens all opened on July while today was still 30 June. latestDataMonth() now returns the latest month that has actually STARTED (<= the current month), so a future-dated invoice no longer drags the default ahead; it only falls back to a future month if ALL data is future-dated. Verified on the 30-Jun backup (which has a 1-Jul invoice): default month is now June. 2 regression tests added (1078 total).
Version 6.232.0 - Cloud-authoritative storage with a versioned, self-refreshing local cache. The local key is now versioned (blackstars-crm-v2), so a deployment ignores any stale data left by an older version - it can never be read as authoritative nor overwrite the cloud. After every CONFIRMED server read the local emergency cache is overwritten with the cloud data and superseded legacy keys are removed (overwrite-after-good-server-load), so the offline copy always reflects the last good cloud state. To avoid stranding anyone on first run after deploy, localStorage.load falls back to the previous key ONCE if the new key is empty - emergency read-only, never written back to the cloud. The cloud (Firebase) remains the single source of truth for every save: server-source reads, and ALL cloud writes stay blocked until a real server read is confirmed (cloudReadFailed guard) or an explicit restore/clear. No cookies or Firebase Auth are touched. Validated with a localStorage + Firebase mock: legacy fallback, save-writes-v2, v2-preferred-over-legacy, cloud-read-refreshes-v2, and stale-legacy-removed.
Version 6.231.0 - The Reports / Visual performance dashboard now uses the SAME billed basis as the Monthly Report and Dashboard, so every financial screen finally shows the same numbers. It was previously on a CASH basis (payment dates) with a different salary figure (actually-paid), which made its revenue/profit disagree with the other screens. Now: revenue = billedInPeriod (invoice value by billing month, line-aware), salaries = salariesEarnedInPeriod (the auto-calculated cost, same single source of truth), P&L expenses exclude salary-category settlements, and revenue-by-category / revenue-by-sport are billed and normalized so they re-sum exactly to revenue (discount-safe). Validated on real June data: Reports revenue 104,455, salaries 5,803, profit 69,349 - identical to the Monthly Report (diff 0). Also fixed the Owner Dashboard 'top sports by revenue' to be line-month aware. New period-aware helpers: billedInPeriod, billedByCategoryInPeriod, billedBySportInPeriod, salariesEarnedInPeriod, allDataMonths. 2 regression tests added (1076 total).
Version 6.230.0 - Invoices can now span months: a sport added in a later month keeps the ORIGINAL invoice (original month + issue date) but its revenue counts in the month it was added, and the invoice shows up under that month's search too. Implemented in stages, each validated on real June data with ZERO change to existing single-month numbers (Dashboard 104,455 / 35,106 / 5,803 / 69,349, reconciliation leakage 0). STAGE A: new per-line billMonth + line-aware billedInMonth/collectedInMonth/dueInMonth (each invoice contributes its price-share to each month; share is 1 for single-month invoices so nothing changes); invoiceMonths/invoiceTouchesMonth make an invoice appear under every month it bills in (month dropdowns + invoice search + Dashboard/Monthly Report/Reconciliation all updated, staying aligned). STAGE B: a brand-new sport joins the member's current active membership invoice (one invoice per member) and carries its own billMonth so its revenue lands in the add-month; the document stays in its original month. Renewals continue to generate a SEPARATE invoice for the renewed sport (existing behaviour, confirmed). So: enroll X (Jun) + Y (Jun) then renew X (Jul) = two invoices (X+Y, and renewed X); enroll X (Jun) then add Y (Jul) = ONE June invoice whose Y revenue counts in July and which appears in both June and July search. Due stays per-activity (price - discount - that sport's paid) and therefore naturally lands in each activity's bill month. 7 regression tests added (1074 total).
Version 6.229.0 - Invoice issue date is now preserved and per-line. PROBLEM: adding a second sport (e.g. Summer Camp) later was rewriting the printed issue date of the first sport (Gymnastic enrolled 9 Jun was showing 'Issued 29 Jun', the day camp was added). FIX: each line item now prints ITS OWN issue date - an explicit per-line issueDate, else that sport's subscription start, else the invoice date (new lineIssueDate helper) - so Gymnastic shows 9 Jun and Summer Camp shows 29 Jun, with no data migration needed. The invoice's original issue date (inv.date) is never changed when a sport is added; a 'Last updated' date is now shown in the header when the invoice has been modified after issue. Edit pricing stamps each newly added line with its own issueDate and records inv.lastUpdated. (Payment-history table on the invoice shipped in 6.228.) 3 regression tests added (1067 total).
Version 6.228.0 - Edit pricing & payment reworked so two sports' partial payments never mix, plus per-collection payment history on the invoice. (1) Every payment recorded through Edit pricing is now TAGGED with its sport; per-sport Paid-so-far reads those tagged payments (new invoicePaidForSport helper), and only legacy untagged payments fall back to a price-share split. So paying 400 for Gymnastic then 200 for Summer Camp shows 400 / 200, not a mixed 329 / 271. (2) A Review & confirm step now appears on Save, showing each sport's price, the new payment, paid and due before anything is written. (3) Payment method (cash/card/Fawran/transfer) is now chosen on the screen and stored with each collection. (4) The invoice PDF prints a Payments received history table (date - method - sport - amount) with the total paid. RULE enforced: one membership invoice per member per MONTH - a new sport joins the existing same-month invoice; a fresh invoice is only created when that month has none (never split within a month). 4 regression tests added (1064 total).
Version 6.227.0 - CRITICAL FIX: invoice now shows the member's REAL subscription end date. The printed invoice (both the per-line 'Valid: ... -> ...' and the red 'Finish your classes by' banner) was recomputing a Summer Camp's end date from its duration LABEL (e.g. '1 month' -> start + 30 days) instead of using the stored end date. So after editing a camp's validity window - which correctly updated the subscription end (e.g. to 28 Aug) but left the duration label as '1 month' - the member modal showed 28 Aug while the invoice still showed 29 Jul. Added subscriptionValidEnd(), which trusts the stored end (exactly what the member modal shows) and only derives a date from the label / validity-days when the stored end is missing or invalid. The invoice and the member screen now always agree. 4 regression tests added (1060 total).
Version 6.226.0 - Salary expenses now support external / ad-hoc coaches. On the Expenses screen (category Salary) the Coach drop-down is now OPTIONAL and there is a free-text field for an external coach who is not a club employee. Pick a registered coach (the payment settles his auto-calculated salary) OR type an external name. Accounting is handled correctly: a registered coach has an auto-calculated cost so his payment is a settlement (excluded from P&L); an external coach has NO auto-calculated pay, so his payment IS the cost and is added to the month's salary cost (salariesEarnedInMonth) and to the Salaries report under a new 'Ad-hoc / external coaches' section. Existing un-named Salary expenses are unaffected (still treated as settlements), so aligned totals don't move. Reconciliation still balances to zero. 5 regression tests added (1056 total).
Version 6.225.0 - CRITICAL FIX: Generate latest invoice now bills EVERY enrolled sport, not just one. The enrollment filter required a coach (e.sport && e.coachId), so any coachless enrollment - e.g. Summer Camp, which has no coach - was silently dropped from the invoice. A member enrolled in Gymnastic (425, coach Jennifer) + Summer Camp (350, no coach) was being invoiced 425 instead of 775. Fixed in all three places (generateInvoiceForMember, the Generate-latest-invoice modal, and its live preview) to include any enrolled sport with a price or classes regardless of coach; this now matches memberRenewalValue. 3 regression tests added (1051 total).
Version 6.224.0 - Log salary payments from the Expenses screen + fixed a salary double-count. The expense form now shows a Coach drop-down when the category is Salary; that entry is recorded as a payment/advance to the chosen coach and appears in his salary report (Net pay shows - Advance and - Paid via expenses, reducing what is still owed). Crucially, Salary-category expenses are now EXCLUDED from the P&L expense total - they are settlements of the auto-calculated salary cost, not an extra expense - which removes a pre-existing DOUBLE-COUNT (the club had 6,470 QAR of Salary expenses being added on top of the 5,803 auto-calculated cost). Net effect on real June data: expenses corrected 41,576 -> 35,106 and net profit 62,879 -> 69,349, identical on the Dashboard and Monthly Report. The Reconciliation still counts salary cash payments as a cash outflow (new Salaries-paid-cash slice on the Financial Overview bar) so cash reconciles to zero. salariesPaidInMonth now includes these expense payments as paid-so-far. 6 regression tests added (1048 total).
Version 6.223.0 - Salaries are now ONE auto-calculated number across every report, removing the last financial inconsistency. Added salariesEarnedInMonth() (the auto-calculated salary COST = each active coach's fixed pay + commission) and used it as the single salary figure on the Main Dashboard, Monthly Report and Financial Overview - previously the Dashboard showed earned salary while the Monthly Report showed salary actually paid, so expenses (41,576 vs 38,772) and net profit disagreed. Net profit is now computed on the BILLED (invoice) basis everywhere, so the Dashboard and Monthly Report show the same revenue, expenses, salaries and net profit; the Monthly Report headline now reads Revenue (billed) with collected and due shown beneath it (billed = collected + due). salariesPaidInMonth() is kept as a separate cash sub-figure. Validated on real data: revenue 104,455, expenses 41,576, salaries 5,803, net profit 62,879 identical across screens; reconciliation collected 94,130 with leakage 0. 7 more alignment tests (1042 total).
Version 6.222.0 - Aligned every financial screen to read revenue from invoices on one consistent basis, fixing the mismatched numbers across Dashboard / Owner Dashboard / Monthly Report / Reconciliation / Insights. BILLED (sum of invoice amounts in the billing month) is now the single revenue figure on the Main Dashboard, Owner Dashboard, Invoices and Transactions; COLLECTED (paid against those same invoices) drives the Monthly Report and Reconciliation - so billed = collected + due holds on every screen. The Main Dashboard previously summed cash by payment date (counting prior-month invoices paid this month), which is why it disagreed; it now uses the billing-month billed amount. Also FIXED the Reconciliation leakage bug: the accounted-for total was adding TOTAL expenses where only CASH expenses come out of collected cash (non-cash expenses are paid from the bank), which produced a large phantom leakage - it now reconciles to zero. 8 alignment regression tests added (1035 total).
Version 6.221.0 - Storage hardened so Firebase is the single source of truth and data can't be wiped: (1) the app now reads from the Firebase SERVER on startup (never the local/cached copy), so it can't load a stale/empty state and overwrite good cloud data; (2) if the server can't be reached, the last copy is shown READ-ONLY and ALL cloud writes are blocked (loud orange banner) until reconnected - localStorage is now a non-authoritative emergency copy only and can never overwrite the cloud; (3) the data-loss guard is strengthened from total==0 to per-collection: a save is refused whenever invoices OR members drop to zero, or either collapses by >90% (explicit Restore/Clear still bypass via __allowEmptySave). 9 guard unit-tests added.
Version 6.220.0 - NEW Financial Overview screen (Insights, admin) - one simple screen that shows where every riyal collected from invoices goes: distribution bar across cash-in-hand, cash collected by owner, bank (card/transfer/Fawran) and cash expenses; collected-by-method breakdown (cash / card / transfer / Fawran); a cash-tracking card (cash collected -> owner took / cash expenses / cash in hand) with a reconciliation check; a money-out-vs-in card (collected, expenses, salaries, net); and the live Due total. Also FIXED a salary undercount: a 'paid' salary record stores snapshotNet (not amount/paid), so the Monthly Report was missing net payouts - added a canonical salariesPaidInMonth() helper used by the Monthly Report, the new overview and the reconciliation, so salaries (kept separate from expenses) are now counted correctly. 3 new salary logic tests (1027 total).
Version 6.219.0 - Financial audit pass (expenses, salaries, commissions, dashboards). Fixes: (1) Commission/payroll consistency - coachEarnings and computeMonthlyPay now scope invoices by the canonical billing month (month, falling back to the date) instead of the raw month field, so an invoice that has a date but no month is no longer counted in revenue while being missed in coach commission. Added a migration that backfills month from date on every invoice (and expense) so all screens agree. (2) Overpayment no longer breaks the books: collectedInMonth now caps each invoice's collected amount at its billed amount, so the identity billed = collected + due always holds (an imported/edited overpaid invoice could previously make Collected exceed Billed). Verified across an adversarial dataset: per-invoice integrity, billed=collected+due, line-share re-sum=amount, memberOutstanding=sum of invoice balances, and MonthlyReport.revenue=sum of in-month payments all reconcile. Added 4 revenue-identity logic tests (1024 total). Note: the main Dashboard's Total Revenue is intentionally cash-collected-this-month (documented in its tooltip), distinct from the billed totals on Owner Dashboard / Club Revenue / Transactions / Invoices.
Version 6.218.0 - NEW Advice & Articles (broadcast) feature (Activities section, icon, route 'posts'). Coaches can publish advice to their own students - to ALL of them, by SPORT, or a hand-picked SELECTION. Admins can publish an advice/article to EVERYONE, by SPORT, or to SPECIFIC people, with an optional 'also notify coaches' toggle. Recipients (members and coaches) get a bell notification, the post auto-marks as read when they open the screen, and they can REPLY in a thread. Every post tracks WHO has read it (read X/Y with an expandable read / not-yet list), visible to the author and to admins. Admins see all coach advice read-only. New state.posts collection (synced), helpers (resolvePostRecipients / postIsForUser / markPostRead / unreadPostsForUser / publishPost), bell-notification integration, and 12 new logic-test assertions.
Version 6.217.0 - (1) Critical attendance bug fixed: a renewed (Active) member was wrongly warned that they had already finished all their classes when taking attendance. The over-limit check was counting ALL-TIME present marks for the sport against the CURRENT subscription's class count; it now counts only within the current subscription's window (start to end), so a renewal resets the count. The warning is also now bilingual. (2) Arabic localization: added a central TOAST_AR translation map wired into toast() so common status messages (Member not found, Admins only, Saved, Refreshed, Enter a valid amount, etc.) localize automatically when the app is in Arabic, without editing each call site.
Version 6.216.0 - Invoice PDF made fully bilingual and more polished for Arabic-speaking customers: added a professional Arabic web font (Tajawal) so Arabic renders cleanly instead of a default system font; added the Arabic club name in the header and footer; made the remaining English-only labels bilingual (Description/الوصف column header, payment-status badges Paid/مدفوعة · Unpaid/غير مدفوعة · Partially paid/مدفوعة جزئياً, and the footer thank-you/contact). Totals, parties, period, activity, terms and validity were already bilingual; combined with the Arabic month names this gives a clean dual-language invoice end to end.
Version 6.215.0 - Invoice PDF: the Arabic side now shows Arabic month names (يناير، فبراير، مارس، أبريل، مايو، يونيه، يوليه، أغسطس، سبتمبر، أكتوبر، نوفمبر، ديسمبر) for the validity (صالح), issued (صدرت) and period (الفترة) lines, instead of English abbreviations. Added fmtDateAr/fmtMonthAr helpers. Also enlarged the validity-notice text (the "classes must be attended by the end date / unused classes are forfeited" banner) for clarity.
Version 6.214.0 - Frozen members are no longer flagged as "about to expire". The notification bell (admin "memberships expiring soon", coach "students expiring soon", and a member's own "membership expiring soon") now skips frozen (and withdrawn) members, and the Members table expiry column shows "⏸ frozen" instead of a "⚠ Xd" countdown for them. The Reminders screen, Expiring screen and dashboard alerts already excluded frozen members.
Version 6.213.0 - NEW Birthdays screen (Membership section, 🎂): lists members with birthdays by range (Today / next 7 days / this month / next 90 days), showing the member, birthdate, when, age they're turning, and phone, with a one-tap "Send wishes" button that opens WhatsApp (wa.me) with a bilingual celebration message pre-filled — staff just press send. Members with no/placeholder WhatsApp number show no button. The existing 🎂 birthday card on the dashboard now opens this screen instead of the Members list.
Version 6.212.0 - Sortable columns everywhere: the generic click-to-sort header (with the up/down icon) already enhanced tables on page load, but tables that rebuilt on a filter change, opened inside a modal, or loaded asynchronously were missed. Added a debounced MutationObserver that re-applies the (idempotent) sortable-header enhancement whenever new tables appear, so every data table's columns are now sortable. Action/empty columns and inputs are still skipped; tables with their own sort (e.g. Members) are left as-is.
Version 6.211.0 - FIX (Edit pricing / record payment on the member card): for a member enrolled in two+ sports on ONE invoice, every sport row showed the FULL invoice total (and paid/discount) instead of that sport's own cost. Now each row shows its own line-item price, with the invoice discount and paid apportioned per sport. Also fixed a save-side corruption: saving a multi-sport invoice used to overwrite the whole invoice amount/paid with a single sport's value (last row wins) — it now sets each sport's line price and rolls the invoice up correctly (amount = sum of lines; discount + paid = sum across sports).
Version 6.210.0 - CRITICAL FIX: frozen / non-completed members were being paid FULL coach commission instead of attendance-prorated, whenever their membership invoice line carried no sport/class-count (so no subscription matched and the "no total" fallback paid full). Now when a line doesn't map to a subscription, attended/total is derived from the member's own subscription(s) for that coach, so Active/Frozen members pro-rate correctly (e.g. 3/8 attended on a 1,125 fee -> 422 base, not 1,125). Only members with genuinely no class-based subscription fall back to full. Revenue-detail report shows attended/total and the prorated marker for all prorated lines, not just frozen.
Version 6.209.0 - (1) Monthly Report "Expenses" now shows its makeup as a subtext (X expenses + Y salaries) — it has always included salaries for the profit calc, whereas the Expenses screen lists only expense entries; this makes the two reconcile. (2) Expenses and Invoices filters now PERSIST (sessionStorage) so deleting a row no longer resets your filters. (3) Expenses and Invoices month filter now defaults to the CURRENT month (your last choice is remembered for the session); month dropdowns always include the current month and show the active selection.
Version 6.208.0 - Transactions: when "Has due" is on, the category chips and headline now show the OUTSTANDING due per category (net due bold, gross beside it) plus billed as secondary — so filtering to has-due reads the owed amount, not the billed total. Also fixed net-due totals not resolving in the footer/header (grandNetDue was undefined in net mode).
